...
results = []
for dim in [256, 512, 32, 64, 128, 16]:
    for norm in ["normalize", "standardize"]:
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=0.005,
                                 weight_decay=1e-4)
        best  = 1e9
        for neighbours in [10, 15, 20]:
            for method in ["heat", "power"]:
                ...
                for epoch in range(max_epochs):
                    ...
                    loss +=  - torch.mean((1. - EPS) * (torch.log(1.- q_neg)  ))
                    loss.backward()
                    optimizer.step()
                    ...
                    if loss < best:
                        ...
                        best_t = epoch
                        cnt_wait = 0
                        torch.save(model.state_dict(), 'best_gnumap_' + str(method) + '_neigh' + str(neighbours) + '_dim' + str(dim) + '.pkl')
                    else:
                        cnt_wait += 1
                ...
                model.load_state_dict(torch.load('best_gnumap_' + str(method) + '_neigh' + str(neighbours) + '_dim' + str(dim) + '.pkl'))
                print("=== Evaluation ===")
                ...
                logreg, best_train_acc, best_val_acc, eval_acc, best_epoch2 = node_prediction2(embeds.detach(),
                                                   ...
                                                   lr=0.01, wd=1e-4,
                                                    max_epochs=2000)
                print("done with the third node prediction")
                ...
                print([ method, neighbours, norm, dim,  acc_train, val_train, acc,
                   acc_train_default, acc_val_default, acc_default, other['F1Mi'], other['F1Ma'],   other['acc']])
                ...
                results += [[ method, neighbours, norm, dim, train_roc, train_ap,
                   test_roc, test_ap, acc_train, val_train, acc,
                   acc_train_default, acc_val_default, acc_default, epoch, other['F1Mi']['mean'], other['F1Mi']['std'],
                   other['F1Ma']['mean'],other['F1Ma']['std'],
                   other['acc']['mean'], other['acc']['std']]]
                pd.DataFrame(np.array( results), columns =[ 'method', 'neighbours', 'norm',
                                                           'dim', 'train_roc', 'train_ap',
                   'test_roc', 'test_ap', 'acc_train', 'val_train', 'acc',
                   'acc_train_default', 'acc_val_default', 'acc_default', 'epoch', 'F1Mi', 'F1Ma','F1Mi-mean',
                   'F1Mi-std','F1Ma-mean','F1Ma-std','acc-mean','acc-std'] ).to_csv("/Users/cdonnat/Downloads/res_second_experiment3.csv")
None;

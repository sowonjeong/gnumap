    ...
    for norm in ["normalize", "standardize", "none" ]:
        ...
        for neighbours in [10, 15, 30, 50]:
            for method in ["heat", "power", "laplacian"]:
                ...
                new_data = Data(x=data.x, edge_index=edge_index, y=data.y, edge_attr=edge_weights)
                transform = RandomLinkSplit(num_val=val_ratio, num_test=test_ratio,
                                is_undirected=True, split_labels=True)
                ...
                for epoch in range(150):
                    ...
                    edge_weights_pos = train_data.edge_attr[:len(train_data.pos_edge_label)]
                    edge_weights_pos = fast_intersection(row_pos, col_pos, edge_weights_pos,
                    ...
                    edge_weights_neg = fast_intersection(row_neg, col_neg, EPS * torch.ones(len(row_neg)),
                                     target, unknown_dist=1.0, far_dist=5.0)
                    ...
                    if epoch % 10 == 0 :
                                ...
                                pd.DataFrame(np.array( results), columns =[ 'method', 'neighbours', 'norm',
                                   ...
                                   'test_roc', 'test_ap', 'acc_train', 'val_train', 'acc',
                                   'acc_train_default', 'acc_val_default', 'acc_default', 'epoch'] ).to_csv("/Users/cdonnat/Downloads/res_semi_supervised.csv")
None;

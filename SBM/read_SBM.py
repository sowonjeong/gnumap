...
def readSBM(type = 'lazy', features = None):
    ...
    if features == 'ones':
        new_data = Data(x=torch.ones(500, 500), y = torch.Tensor(y.values).T, edge_index=torch.tensor(data.values, dtype=torch.long).T)
        new_data.edge_weight = torch.ones(new_data.edge_index.shape[1])
    else:
        new_data = Data(x=torch.eye(500), y = torch.Tensor(y.values).T, edge_index=torch.tensor(data.values, dtype = torch.long).T)
        new_data.edge_weight = torch.ones(new_data.edge_index.shape[1])
